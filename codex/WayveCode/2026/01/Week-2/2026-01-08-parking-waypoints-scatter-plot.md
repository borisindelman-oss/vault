...
- Added live Bokeh server mode (no file) when `--output_path` is omitted, with on-demand frame loading, index slider with timestamp display, and prev/next buttons (pre-sized when length is available).
