...
- **Current priorities:**
  - Verify stopping_mode + hazard window behavior in OTF batches
- **Blockers:**
...
- **Success criteria:**
  - Parking model accepts `stopping_mode` input (1=NA, 2=PARK, 3=PUDO)
...
## Design
- **Approach:**
  - Add DataKeys.STOPPING_MODE and a new ST adaptor with 3-state embedding (NA/PARK/PUDO).
  - Extend parking model config + ST model builder to optionally include stopping_mode adaptor.
  - Stage route shortening: replace map route polyline with a truncated segment ending near the gear=0 transition (with jitter).
  - Extend ParkingDeploymentWrapperImpl to derive stopping_mode from DILC (OFF=park, ON=pudo), with optional override if stopping_mode already provided in inputs (test path).
- **Key decisions:**
...
sequenceDiagram
  ...
  WRAP->>MODEL: stopping_mode (1=NA,2=park,3=pudo)
...
- **Stage 3: Connect DILC → stopping_mode in parking wrapper**
  ...
  - **Status:** Implemented (2026-01-22). DILC is included in parking driving_controls and mapped to stopping_mode in the wrapper.
- **Stage 4: OTF stopping_mode targets (hazard-based + random mix)**
  ...
  - **Status:** Implemented (2026-01-22). Hazard is derived from `ground_truth__state__vehicle__indicator_light` within ±10s of the entry frame.
...
- **2026-01-22:** Stop-index route shortening jitter: end point is up to 0–20m before the stop (interpolated point).
- **2026-01-22:** Parking wrapper default: when DILC not present, stopping_mode defaults to PUDO.
- **2026-01-22:** Parking wrapper decouples indicator gating via `use_dilc_indicator_state` (default False).
...
- **Current DILC flow (main):**
  ...
  - Parking wrapper now includes DILC in driving_controls keys and maps it to stopping_mode; indicator gating is disabled by default for parking (`use_dilc_indicator_state=False`).
