...
- 2026-02-12 Parking wrapper TorchScript follow-up:
  - Next failure (`magenta-wolverine-topographical-125575`) hit TorchScript compilation in `ParkingDeploymentWrapperWithRadar._add_driving_controls_inputs(...)` with:
    - `... do not support break or continue inside the body of these loops`
  - Root cause was `continue` in the parking-wrapper `dilc_mode` branch.
  - Updated parking-wrapper control loop to remove `continue` while preserving semantics (`dilc_mode` branch now explicit no-op).
  - Validation:
...
- Reference: [[agent_tasks/2026/02/Week-2/2026-02-12-pudo-parking-wrapper-torchscript-continue-fix]]
